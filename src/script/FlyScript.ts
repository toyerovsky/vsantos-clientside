...
export default class FlyScript implements IScript {
    ...
    private _freeCamHandler = (...args: any[]) => {
        ...
        mp.players.local.position = new mp.Vector3(this.flyCamera.position.x, this.flyCamera.position.y, this.flyCamera.position.z - 2.5);
        ...
        {
            var oldPosition: MpVector3 = this.flyCamera.position;
            var newx = -(Math.sin(yradian) * speed);
            var newy = Math.cos(yradian) * speed;
            var newz = Math.sin(xradian) * speed; // up or down					
            this.flyCamera.position = new mp.Vector3(oldPosition.x + newx, oldPosition.y + newy, oldPosition.z + newz);
